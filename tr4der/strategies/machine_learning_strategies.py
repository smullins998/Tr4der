...
class MachineLearningStrategies:
    ...
    def linear_regression(df: DataFrame, lags: int = 5) -> None:
        ...
        test["Drawdown"] = (
            test["Cumulative_Return"] / test["Cumulative_Return"].cummax()
        ) - 1
    ...
    def svm_regression(
        ...
        df = MachineLearningStrategies._calculate_technical_indicators(
            ticker, df, technical_indicators
        )

        # Split the data into training and testing sets
        train_data, test_data = train_test_split(
            df, test_size=0.5, random_state=0, shuffle=False
        )
        ...
        test_data[f"{ticker}_signal"] = np.where(
            test_data["predictions"] > 0, "Buy", "Sell"
        )
        test_data[f"{ticker}_position"] = np.where(
            test_data[f"{ticker}_signal"] == "Buy", 1, -1
        )
        test_data[f"Total_Return"] = (
            test_data[f"{ticker}_position"] * test_data[f"{ticker}_return"]
        )
        test_data[f"Cumulative_Return"] = (1 + test_data[f"Total_Return"]).cumprod()
        test_data[f"Drawdown"] = (
            test_data[f"Cumulative_Return"] / test_data[f"Cumulative_Return"].cummax()
        ) - 1
    ...
    @staticmethod
    def nearest_neighbors_regression(
        df: DataFrame,
        technical_indicators: List = ["SMA_20", "SMA_50", "EMA_20", "EMA_50", "RSI_14"],
    ) -> None:
        "We will only support one ticker for now"
    ...
    def neural_network(
        ...
        hidden_layers: Tuple[int, int] = (100, 100),
    ) -> None:
        "We will only support one ticker for now"
        ...
        # Create and train the SVR model
        mlp = MLPRegressor(
            hidden_layer_sizes=hidden_layers, max_iter=max_iterations, shuffle=False
        )
    ...
    def LSTM(
        ...
        test_data[f"{ticker}_signal"] = np.where(
            test_data["predictions"] > test_data[ticker].shift(1), "Buy", "Sell"
        )
        test_data[f"{ticker}_position"] = np.where(
            test_data[f"{ticker}_signal"] == "Buy", 1, -1
        )
